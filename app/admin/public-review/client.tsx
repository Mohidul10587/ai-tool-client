...
import { updatePublicReviewStatus, updatePublicReviewRequest } from "@/lib/public-review-actions";
...
export default function AdminPublicReviewClient({ requests: initial }: { requests: Request[] }) {
  ...
  const handleApprove = async (req: Request) => {
    setLoading(req.id);
    await updatePublicReviewStatus(req.id, "approved");
  ...
  const handleReject = async (req: Request) => {
    setLoading(req.id);
    await updatePublicReviewStatus(req.id, "rejected", rejectNote);
  ...
  const handleEdit = async () => {
    ...
    await updatePublicReviewRequest(editReq.id, { url: editUrl, tool_name: editToolName });
  ...
  return (
    <div className="p-6">
      <h1 className="mb-6 text-2xl font-bold text-gray-900">Public Review Requests</h1>
      ...
      <Dialog open={!!editReq} onOpenChange={(v) => { if (!v) setEditReq(null); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Public Review Request</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div className="space-y-1">
              <Label htmlFor="pr-name">Tool Name</Label>
              <Input id="pr-name" value={editToolName} onChange={(e) => setEditToolName(e.target.value)} placeholder="Tool Name" />
            </div>
            <div className="space-y-1">
              <Label htmlFor="pr-url">URL</Label>
              <Input id="pr-url" type="url" value={editUrl} onChange={(e) => setEditUrl(e.target.value)} placeholder="https://..." />
            </div>
