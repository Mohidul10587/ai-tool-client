...
import { updateUserPublicReviewRequest, deleteUserPublicReviewRequest } from "@/lib/public-review-actions";
import { UserEditModal } from "@/components/user-edit-modal";
import { ConfirmDialog } from "@/components/confirm-dialog";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";

type Request = { id: number; url: string; tool_name: string; price_paid: string; status: string; rejection_note: string | null; submitted_at: string };

const statusVariant = (s: string) =>
  s === "approved" ? "default" : s === "rejected" ? "destructive" : "secondary";

export function DashboardPublicReviewClient({ requests: initial }: { requests: Request[] }) {
  ...
  async function handleDelete() {
    ...
    await deleteUserPublicReviewRequest(deleting.id);
  ...
  return (
    <>
        ...
        {requests.map((req) => (
          <div key={req.id} className="rounded-lg border p-4 flex items-start justify-between gap-4 bg-white">
            <div className="flex-1 min-w-0 space-y-1">
              ...
              {req.status === "approved" && (
                <p className="text-xs text-emerald-700 bg-emerald-50 px-3 py-2 rounded-md">Your review has been approved and will be published shortly.</p>
              )}
            </div>
      ...
      <UserEditModal item={editing} onClose={() => setEditing(null)} onSave={(id, data) => updateUserPublicReviewRequest(id, data)} />
      <ConfirmDialog
        open={!!deleting}
