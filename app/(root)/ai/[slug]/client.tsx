...
export function ToolDetailsClient({
  ...
  const voteProps = { toolId: tool.id, initialUpvotes, initialDownvotes, initialUserVote };

  const LogoBox = ({ size }: { size: "sm" | "md" }) => {
    const [imgError, setImgError] = useState(false);
    return (
      <div className={`${size === "md" ? "w-16 h-16" : "w-14 h-14"} bg-gray-100 rounded-xl flex items-center justify-center shrink-0 overflow-hidden border border-gray-200`}>
        {tool.logo_url && !imgError ? (
          ...
          <span className={`text-gray-700 font-bold ${size === "md" ? "text-2xl" : "text-xl"}`}>{tool.name?.[0]}</span>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-white relative">
      ...
      <div className="pb-16 lg:pb-12 flex">
        <div className="hidden lg:block"><Sidebar ads={featuredAds.slice(0, 10)} /></div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  {tool.subcategory_snapshot && (
                    <span className="px-2.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-700 rounded">{tool.subcategory_snapshot.toUpperCase()}</span>
                  )}
                  {tool.pricing && (
                    <span className="px-2.5 py-0.5 text-xs font-medium rounded-full bg-black text-white">{tool.pricing.toUpperCase()}</span>
                  )}
                </div>
                <p className="text-gray-600 text-sm">{tool.short_description ?? tool.overview?.slice(0, 160)}</p>
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                      ...
                      <ul className="space-y-2">
                        {pros.map((p, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm text-gray-700"><span className="text-gray-400 mt-0.5">•</span>{p}</li>
                        ))}
                      </ul>
                    </div>
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                      ...
                      <ul className="space-y-2">
                        {cons.map((c, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm text-gray-700"><span className="text-gray-400 mt-0.5">•</span>{c}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
        ...
        <div className="hidden lg:block"><Sidebar ads={featuredAds.slice(10, 20)} /></div>
      </div>
    </div>
  );
}
