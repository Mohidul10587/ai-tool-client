...
type Tool = {
  ...
  pricing_info: { model: string; paidFrom: string; billingFrequency: string; freeTrial?: string } | null;
  pros: string[] | null;
  cons: string[] | null;
...
const FEATURE_ICONS = [
  <Sparkles className="h-5 w-5" />, <Zap className="h-5 w-5" />, <Layers className="h-5 w-5" />,
  <Palette className="h-5 w-5" />, <Wand2 className="h-5 w-5" />, <ImageIcon className="h-5 w-5" />,
];
...
function AdSpot({ ads }: { ads: Ad[] }) {
  ...
  if (!ads.length) return (
    <div className="flex h-full w-full flex-col items-center justify-center rounded-xl border border-black/10 bg-gray-50 px-3 py-2">
      <div className="h-8 w-8 rounded-lg bg-black/10 text-[9px] font-medium text-black/30 flex items-center justify-center">Logo</div>
  ...
  return (
    ...
    >
      <div className="text-center">
        <div className="mx-auto flex h-8 w-8 items-center justify-center rounded-lg bg-black/10 text-[9px] font-medium text-black/50">Logo</div>
        <div className="mt-1.5 text-sm font-bold text-black line-clamp-1">{ad.description}</div>
...
function FeaturedAdsSidebar({ ads }: { ads: Ad[] }) {
  ...
  return (
    <div className="hidden lg:block">
      <div className="fixed left-2 top-[52px] flex h-[calc(100vh-52px)] w-48 flex-col gap-2 py-2 xl:left-3 xl:w-52">
        {slots.slice(0, 5).map((slotAds, i) => <div key={i} className="flex-1"><AdSpot ads={slotAds} /></div>)}
      </div>
      <div className="fixed right-2 top-[52px] flex h-[calc(100vh-52px)] w-48 flex-col gap-2 py-2 xl:right-3 xl:w-52">
        {slots.slice(5, 10).map((slotAds, i) => <div key={i} className="flex-1"><AdSpot ads={slotAds} /></div>)}
      </div>
    </div>
  );
}
...
// ── Vote Button ──────────────────────────────────────────────────────────────
function VoteButton({ initialVotes = 0, size = "default" }: { initialVotes?: number; size?: "default" | "small" }) {
  const [votes, setVotes] = useState(initialVotes);
  const [userVote, setUserVote] = useState<"up" | "down" | null>(null);

  const handleVote = (e: React.MouseEvent, type: "up" | "down") => {
    e.preventDefault(); e.stopPropagation();
    if (userVote === type) { setVotes(type === "up" ? votes - 1 : votes + 1); setUserVote(null); }
    else if (!userVote) { setVotes(type === "up" ? votes + 1 : votes - 1); setUserVote(type); }
    else { setVotes(type === "up" ? votes + 2 : votes - 2); setUserVote(type); }
  };
  ...
  return (
    <div className="flex flex-col items-center border border-gray-200 rounded-lg overflow-hidden">
      <button onClick={(e) => handleVote(e, "up")} className={`transition-colors w-full flex items-center justify-center px-2 py-1 hover:bg-gray-50 ${userVote === "up" ? "text-black" : "text-gray-400"} ${isSmall ? "text-xs" : "text-sm"}`}>▲</button>
      <span className={`font-bold text-black px-2 ${isSmall ? "text-xs" : "text-sm"}`}>{fmt(votes)}</span>
      <button onClick={(e) => handleVote(e, "down")} className={`transition-colors w-full flex items-center justify-center px-2 py-1 hover:bg-gray-50 ${userVote === "down" ? "text-black" : "text-gray-400"} ${isSmall ? "text-xs" : "text-sm"}`}>▼</button>
    </div>
  );
}
...
function CommentItem({ comment }: { comment: Comment }) {
  ...
  const handleVote = (type: "up" | "down") => {
    ...
    if (userVote === type) { newVotes[type]--; setUserVote(null); }
  ...
  return (
    <div className="flex gap-3 py-4 border-b border-gray-100 last:border-0">
      <div className="flex flex-col items-center shrink-0 border border-gray-200 rounded-lg overflow-hidden">
        <button onClick={() => handleVote("up")} className={`text-xs w-full px-2 py-1.5 flex items-center justify-center hover:bg-gray-50 ${userVote === "up" ? "text-black" : "text-gray-400"}`}>▲</button>
        <span className="text-xs font-medium text-black px-2">{votes.up - votes.down}</span>
        <button onClick={() => handleVote("down")} className={`text-xs w-full px-2 py-1.5 flex items-center justify-center hover:bg-gray-50 ${userVote === "down" ? "text-black" : "text-gray-400"}`}>▼</button>
      </div>
        ...
        <div className="mb-8">
          <div className="hidden sm:flex items-start gap-4">
            <div className="w-16 h-16 bg-black rounded-xl flex items-center justify-center shrink-0 overflow-hidden">
              {tool.logo_url ? <img src={tool.logo_url} alt={tool.name ?? ""} className="w-full h-full object-cover" /> : <span className="text-white text-2xl font-bold">{tool.name?.[0]}</span>}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                <h1 className="text-2xl font-bold text-black">{tool.name}</h1>
                {tool.subcategory_snapshot && <span className="px-2.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-700 rounded">{tool.subcategory_snapshot.toUpperCase()}</span>}
                {tool.pricing && <span className="px-2.5 py-0.5 text-xs font-medium rounded-full bg-black text-white">{tool.pricing.toUpperCase()}</span>}
              </div>
              <p className="text-gray-600 text-sm">{tool.overview?.slice(0, 100)}</p>
            </div>
          ...
          <div className="sm:hidden">
            <div className="flex items-start gap-3 mb-4">
              <div className="w-14 h-14 bg-black rounded-xl flex items-center justify-center shrink-0 overflow-hidden">
                {tool.logo_url ? <img src={tool.logo_url} alt={tool.name ?? ""} className="w-full h-full object-cover" /> : <span className="text-white text-xl font-bold">{tool.name?.[0]}</span>}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                    <h3 className="font-semibold text-black text-sm mb-3 flex items-center gap-2">
                      <span className="w-5 h-5 bg-black text-white rounded-full flex items-center justify-center text-xs">+</span>Pros
                    </h3>
                    <ul className="space-y-2">{pros.map((p, i) => <li key={i} className="flex items-start gap-2 text-sm text-gray-700"><span className="text-gray-400 mt-0.5">•</span>{p}</li>)}</ul>
                  </div>
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                    <h3 className="font-semibold text-black text-sm mb-3 flex items-center gap-2">
                      <span className="w-5 h-5 bg-gray-400 text-white rounded-full flex items-center justify-center text-xs">−</span>Cons
                    </h3>
                    <ul className="space-y-2">{cons.map((c, i) => <li key={i} className="flex items-start gap-2 text-sm text-gray-700"><span className="text-gray-400 mt-0.5">•</span>{c}</li>)}</ul>
                  </div>
                </div>
              </section>
            )}
...
function PricingCard({ pricingInfo }: { pricingInfo: { model: string; paidFrom: string; billingFrequency: string; freeTrial?: string } }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      <h2 className="text-lg font-semibold text-black mb-4">Pricing</h2>
      <div className="space-y-3">
        {pricingInfo.model && <div><p className="text-xs text-gray-500 mb-0.5">Pricing model</p><p className="text-sm font-medium text-black">{pricingInfo.model}</p></div>}
        {pricingInfo.paidFrom && <div><p className="text-xs text-gray-500 mb-0.5">Paid option from</p><p className="text-sm font-medium text-black">{pricingInfo.paidFrom}</p></div>}
        {pricingInfo.billingFrequency && <div><p className="text-xs text-gray-500 mb-0.5">Billing frequency</p><p className="text-sm font-medium text-black">{pricingInfo.billingFrequency}</p></div>}
        {pricingInfo.freeTrial && <div><p className="text-xs text-gray-500 mb-0.5">Free Trial</p><p className="text-sm font-medium text-black">{pricingInfo.freeTrial}</p></div>}
      </div>
    </div>
  );
}
...
function ToolInfoCard({ tool }: { tool: Tool }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      ...
      <div className="space-y-3">
        {tool.url && <div><p className="text-xs text-gray-500 mb-0.5">Website</p><a href={tool.url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-black hover:underline">{new URL(tool.url).hostname}</a></div>}
        {tool.platform && <div><p className="text-xs text-gray-500 mb-0.5">Platform</p><p className="text-sm font-medium text-black">{tool.platform}</p></div>}
        {tool.subcategory_snapshot && <div><p className="text-xs text-gray-500 mb-0.5">Category</p><p className="text-sm font-medium text-black">{tool.subcategory_snapshot}</p></div>}
      </div>
    </div>
  );
}
